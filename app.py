...
Entrez.email = "phage-bench-user@example.com"  # NCBI asks that requests self-identify

app = FastAPI(title="Phage Bench")
...
async def run_pipeline(job_id: str, genome: Genome) -> None:
    try:
        ...
        gc = await asyncio.to_thread(gc_content, genome.sequence)

        update_stage(job_id, "gene_calling", "Calling genes with Pyrodigal")
        genes = await asyncio.to_thread(call_genes, genome.sequence)

        update_stage(job_id, "functional_annotation", "Annotating gene function")
        annotation = await asyncio.to_thread(functional_annotation, genes)

        update_stage(job_id, "taxonomy_classification", "Classifying taxonomy")
        taxonomy = await asyncio.to_thread(taxonomy_classification)

        update_stage(job_id, "host_prediction", "Predicting bacterial host")
        host = await asyncio.to_thread(host_prediction)

        update_stage(job_id, "lifestyle_prediction", "Running BACPHLIP (HMM search + classifier)")
        lifestyle = await asyncio.to_thread(lifestyle_prediction, genome)
...
async def submit_job(
    ...
    try:
        if file is not None:
            ...
            genome = await asyncio.to_thread(fetch_from_ncbi, accession)
